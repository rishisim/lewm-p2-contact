#!/usr/bin/env python3
"""Stage 1a latent extraction for Proposal 1.

Reads the existing PushT HDF5 dataset and LeWM object checkpoint, encodes
strided frames with the frozen model, and caches trajectory-split windows.
"""
...
def normalize_action_blocks(action_blocks: np.ndarray, action_mean: np.ndarray, action_std: np.ndarray) -> np.ndarray:
...
def main() -> None:
    ...
    action_mean = finite_actions.mean(axis=0, keepdims=False).astype(np.float32)
    action_std = finite_actions.std(axis=0, ddof=1, keepdims=False).astype(np.float32)
    ...
    if len(valid) < NUM_EPISODES:
        raise RuntimeError(f"Only {len(valid)} usable episodes found, need {NUM_EPISODES}.")

    rng = np.random.default_rng(SEED)
    selected = rng.choice(valid, size=NUM_EPISODES, replace=False)
    split_order = selected.copy()
    rng.shuffle(split_order)
    train_eps = np.sort(split_order[:20]).astype(np.int64)
    test_eps = np.sort(split_order[20:]).astype(np.int64)
    selected_sorted = np.sort(selected).astype(np.int64)

    split_by_ep = {int(ep): "train" for ep in train_eps}
    split_by_ep.update({int(ep): "test" for ep in test_eps})
    ...
    with h5py.File(dataset_path, "r", swmr=True) as h5:
        ...
        for ep in selected_sorted:
            ...
            split_chunks.append(np.full(n_pairs, 0 if split == "train" else 1, dtype=np.int64))
    ...
    np.savez_compressed(
        ...
        selected_episodes=selected_sorted,
    ...
    summary_path = OUT_DIR / "stage1a_summary.json"
    summary_path.write_text(json.dumps(summary, indent=2))

    print("STAGE 1A SUMMARY")
    print(f"episodes: {NUM_EPISODES} (train trajectories={len(train_eps)}, test trajectories={len(test_eps)})")
    print(f"pairs: train={int(train_mask.sum())}, test={int(test_mask.sum())}, total={history.shape[0]}")
