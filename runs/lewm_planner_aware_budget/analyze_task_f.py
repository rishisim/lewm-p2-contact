...
def main():
 ...
 bank_manifest=[]
 for role in ("pilot","fit","selection","evaluation"):
  for path in sorted((WORK/role).glob("*.json")):
   record=json.loads(path.read_text())
   bank_manifest.append({key:record[key] for key in ("role","row_id","episode_id","candidate_seed","candidate_count","valid_count","candidate_tensor_sha256","native_cost_sha256","npz_sha256")})
 by_start={}
 ...
 secondary={}
 for head in ("native","linear"):
  secondary[head]={key:float(np.mean([x[head][key] for x in rows if x[head][key] is not None])) for key in ("top_choice_regret","concordance","spearman","kendall_tau_b","top_10_recall","top_10_jaccard","top_30_recall","top_30_jaccard")}
