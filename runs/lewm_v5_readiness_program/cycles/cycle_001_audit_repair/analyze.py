...
def main() -> None:
    ...
    # This is the first target-array access in cycle 001, after input_seal()
    ...
    centered_errors = np.column_stack(
        [differences[name].mean() - bootstrap[name] for name in co_primary]
    )
    simultaneous_quantile = float(np.quantile(np.max(centered_errors, axis=1), 0.95))
    simultaneous = {
        name: {
            "estimate": float(differences[name].mean()),
            "lower": float(differences[name].mean() - simultaneous_quantile),
            "shared_centered_max_error_quantile_95": simultaneous_quantile,
        }
