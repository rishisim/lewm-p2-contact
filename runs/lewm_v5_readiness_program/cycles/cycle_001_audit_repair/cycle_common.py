#!/usr/bin/env python3
"""Cycle-local constants and atomic artifact helpers."""
...
# Frozen before any new cycle-001 smoke.  These are the repository's tighter
# StagewiseResidualCascade selected-execution tolerances.  The independent
# absolute ceiling is two times the largest consumed cross-batch discrepancy.
NUMERICAL_RTOL = 2e-6
NUMERICAL_ATOL = 2e-7
NUMERICAL_MAX_ABS = 4.76837158203125e-7

EXPECTED_SOURCE_HASHES = {
    "gate_weights": "a9e42f735a55b10e8bdac594cc0cc152cde0b66781fa942749e5c903dc5a33a8",
    "gate_contract": "2a953b7f441098c552fa8d725e581ab570c8692b3747b409004e57844520a869",
...
def sequential_calls(scores: np.ndarray, threshold: float = THRESHOLD) -> np.ndarray:
    ...
    for stage in range(3):
        ...
        active &= values[:, stage] > float(threshold)
...
def verify_expected_sources() -> dict[str, str]:
    paths = {
        "gate_weights": SOURCE_DISCOVERY / "freeze/gate_weights.npz",
        "gate_contract": SOURCE_DISCOVERY / "freeze/gate_contract.npz",
