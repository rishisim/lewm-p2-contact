...
def main() -> None:
    ...
    if seeded_left != seeded_right:
        ...
        count_high = 0
    seeded_rng = np.random.default_rng(1_965_777_771)
    ...
    histogram_rng = np.random.default_rng(1_965_888_881)
    ...
    bootstrap_rng = np.random.default_rng(1_965_999_991)
    ...
    maximum_errors = np.maximum(
        intervals[co_primary[0]]["estimate"] - bootstrap[co_primary[0]],
        intervals[co_primary[1]]["estimate"] - bootstrap[co_primary[1]],
    )
    error_quantile = float(np.quantile(maximum_errors, 0.95))
    simultaneous = {
        name: {
            "estimate": intervals[name]["estimate"],
            "lower": intervals[name]["estimate"] - error_quantile,
        }
