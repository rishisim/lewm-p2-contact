#!/usr/bin/env python3
"""Materialize and seal the complete cycle-001 prospective protocol."""
...
def prior_identifiers() -> tuple[set[int], set[str], dict[str, Any]]:
    ...
    for path in sorted((REPO_ROOT / "runs").rglob("*.json")):
        if PROGRAM_ROOT in path.parents or ".venv" in path.parts:
            continue
        try:
    ...
    for path in (REPO_ROOT / "runs").rglob("*"):
        if path.is_file() and PROGRAM_ROOT not in path.parents and ".venv" not in path.parts:
            strings.add(path.stem)
            strings.add(path.name)
    snapshot = {
        "schema_version": 1,
        "scope": "all parseable prior run JSON outside the V5-readiness program plus prior run filenames; interpreter environments excluded",
...
def build_cohort() -> dict[str, Any]:
        ...
        roles[role] = [
            {
                "role": role,
                "slot": slot,
                "episode_id": f"c001-{role}-{slot:03d}",
    ...
    analysis_seeds = {
        "bootstrap_seed": 1_965_999_991,
        "histogram_seed": 1_965_888_881,
        "seeded_mixture_seed": 1_965_777_771,
    }
    new_numbers = {
        item[key]
    ...
    ledger = {
        ...
        **analysis_seeds,
        ...
        "freshness": "all cycle-001 smoke and prospective episode identifiers and all three RNG streams are disjoint from every recorded prior run identifier",
...
def main() -> None:
    ...
    if seal_path.exists():
        raise RuntimeError("cycle 001 is already sealed")
    ...
    protocol = {
        "schema_version": 1,
        "study": "cycle 001 PlanOracle audit-repair prospective discovery",
        "not_v5": True,
        ...
        "statistics": {
            ...
            "simultaneous_interval": "95th percentile of the maximum centered estimate-minus-bootstrap error across raw and native-whitened analytic co-primary contrasts",
    ...
    preregistration = f"""# Preregistration: LeWM V5-readiness cycle 001 audit repair
...
"""
    ...
    seal = {
        "schema_version": 1,
        "status": "frozen_before_any_cycle_001_smoke_or_prospective_episode",
