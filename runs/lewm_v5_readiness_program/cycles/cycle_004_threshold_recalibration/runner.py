...
def load_gate_tensors(torch: Any, device: Any) -> dict[str, Any]:
    ...
    if gate["threshold"] != THRESHOLD:
        raise RuntimeError("compiled gate threshold drift")
    return gate
...
def score_gate(
    ...
    depth = torch.zeros((len(features), 3), dtype=features.dtype, device=features.device)
    depth[:, stage] = 1
    encoded = torch.cat((features, depth), dim=1)
    raw_score = encoded @ gate["a_raw"] + gate["b_raw"]
    white_score = encoded @ gate["a_white"] + gate["b_white"]
...
def manual_sparse(
    ...
    with torch.inference_mode():
        ...
        for stage, adapter in enumerate(solver.adapters):
            ...
            active = active[local_score > gate["threshold"]]
            if not active.numel():
                break
...
def dense_shadow(
    ...
    with torch.inference_mode():
        ...
        for stage in range(3):
            active &= scores[:, stage] > gate["threshold"]
            calls += active.long()
...
def main() -> None:
    ...
    result = (
        generate(arguments.role)
        if arguments.command == "generate"
        else execute(arguments.role, arguments.device)
    )
