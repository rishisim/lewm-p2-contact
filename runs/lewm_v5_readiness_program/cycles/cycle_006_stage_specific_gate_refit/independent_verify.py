...
def main() -> None:
    ...
    gate_seal = read_json(ROOT / "freeze/gate_freeze_seal.json")
    selection_ledger = read_json(ROOT / "development/gate_selection_ledger.json")
    ...
    with np.load(SOURCE_DISCOVERY / "fit_only_normalization_whitening.npz", allow_pickle=False) as stored:
    ...
    if seeded_left != seeded_right:
        ...
        count_high = 0
    seeded_rng = np.random.default_rng(2_007_777_771)
    ...
    histogram_rng = np.random.default_rng(2_007_888_881)
    ...
    bootstrap_rng = np.random.default_rng(2_007_999_991)
    ...
    checks = {
        ...
        "gate_fit_selection_isolation": gate_freeze["fit_selection_isolation_passed"]
        and not selection_ledger["selected_head_refit_after_selection"]
        and all(
            (REPO_ROOT / relative).exists()
            and sha256_file(REPO_ROOT / relative) == expected
            for relative, expected in gate_seal["files"].items()
        ),
