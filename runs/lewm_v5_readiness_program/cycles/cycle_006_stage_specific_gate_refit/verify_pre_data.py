#!/usr/bin/env python3
"""Read-only verifier for the cycle-006 post-selection pre-data seal."""
...
def verify() -> dict[str, Any]:
    ...
    for raw_path, expected in seal["files"].items():
        ...
        if not path.exists() or sha256_file(path) != expected:
            bad.append(raw_path)
    checks = {
        "status_frozen": seal.get("status") == "frozen_before_any_cycle_006_smoke_or_prospective_episode",
        "fit_count": seal.get("fit_episodes_at_seal") == 240,
        "selection_count": seal.get("selection_episodes_at_seal") == 120,
