...
def main() -> None:
    ...
    roles = {}
    cpu_score_cache = {}
    runtime_thresholds = compiled["thresholds"].astype(np.float32)
    threshold_cast_max_abs = float(
        np.max(np.abs(compiled["thresholds"] - runtime_thresholds.astype(np.float64)))
    )
    for role, expected in (("fit", 240), ("selection", 120)):
        ...
        cpu_calls_float64_thresholds = calls(
            cpu_scores.astype(np.float64), compiled["thresholds"]
        )
        cpu_calls = calls(
            cpu_scores.astype(np.float64), runtime_thresholds.astype(np.float64)
        )
        ...
        roles[role] = {
            ...
            "float64_vs_runtime_float32_threshold_calls_exact": bool(
                np.array_equal(cpu_calls_float64_thresholds, cpu_calls)
            ),
    ...
    try:
        ...
        for device_name in ("cpu", "mps"):
            ...
            thresholds = torch.as_tensor(
                compiled["thresholds"], dtype=torch.float32, device=device
            )
            score_parts = []
            for stage in range(3):
                score_parts.append(
            ...
            active = torch.ones(len(scores_one), dtype=torch.bool, device=device)
            device_calls_tensor = torch.ones(
                len(scores_one), dtype=torch.long, device=device
            )
            for stage in range(3):
                active &= scores_one[:, stage] > thresholds[stage]
                device_calls_tensor += active.long()
            device_calls = device_calls_tensor.cpu().numpy()
            device_checks.append(
                {
                    "device": device_name,
                    "threshold_dtype": str(thresholds.dtype),
                    "threshold_dtype_matches_score_dtype": thresholds.dtype
                    == scores_one.dtype,
    ...
    checks = {
        ...
        "float64_vs_runtime_float32_threshold_calls_exact": all(
            item["float64_vs_runtime_float32_threshold_calls_exact"]
        ...
        and all(
            item["repeat_scores_bitwise_exact"]
            and item["calls_match_compiled_cpu"]
            and item["threshold_dtype_matches_score_dtype"]
    ...
    result = {
        ...
        "runtime_threshold_dtype": "float32",
        "maximum_absolute_threshold_cast_change": threshold_cast_max_abs,
