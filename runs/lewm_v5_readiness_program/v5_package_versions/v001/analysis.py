#!/usr/bin/env python3
"""Presealed V5 confirmation analysis and immutable proposed terminal mapping."""
...
BOOTSTRAP_SEED = 2_051_999_991
HISTOGRAM_SEED = 2_051_888_881
SEEDED_MIXTURE_SEED = 2_051_777_771
...
def input_seal() -> dict[str, Any]:
    seal_path = ROOT / "audit/v5_confirmation_input_seal.json"
    execution_path = ROOT / "data/v5_confirmation_execution.npz"
    execution_manifest_path = ROOT / "data/v5_confirmation_execution_manifest.json"
    raw_manifest_path = ROOT / "data/v5_confirmation_raw_manifest.json"
    execution_manifest = read_json(execution_manifest_path)
    raw_manifest = read_json(raw_manifest_path)
    if (
        len(raw_manifest["episodes"]) != V5_SAMPLE_SIZE
        or execution_manifest["episode_count"] != V5_SAMPLE_SIZE
    ):
        raise RuntimeError("V5 confirmation input is not exactly 1,600 episodes")
    files = {
    ...
    for relative, expected in raw_hashes.items():
        if sha256_file(REPO_ROOT / relative) != expected:
            raise RuntimeError(f"raw V5 confirmation hash drift: {relative}")
    payload = {
        "schema_version": 1,
        "created_unix_ns": time.time_ns(),
        "files": files,
        "raw_episode_hashes": raw_hashes,
        "exact_episode_count": V5_SAMPLE_SIZE,
        "all_1600_raw_and_execution_traces_complete_before_target_array_open": True,
        "raw_manifest_precedes_execution": raw_manifest["created_unix_ns"] < execution_manifest["created_unix_ns"],
        "v5_targets_opened_before_this_seal": False,
    ...
    if seal_path.exists():
        ...
        if existing["files"] != files or existing["raw_episode_hashes"] != raw_hashes:
            raise RuntimeError("existing V5 confirmation input seal mismatch")
        return existing
...
def main() -> None:
    ...
    if sha256_file(execution_path) != execution_manifest["sha256"]:
        raise RuntimeError("V5 confirmation execution hash mismatch")

    # This is the first confirmation target-array access, after input_seal()
    # has durably recorded all 1,600 raw inputs and execution traces.
    ...
    if len(calls) != V5_SAMPLE_SIZE * 38 or not np.array_equal(
        np.unique(episode_ids), np.arange(V5_SAMPLE_SIZE)
    ):
        raise RuntimeError("V5 rows/episode identifiers are not exact")
    ...
    if any(len(values) != V5_SAMPLE_SIZE for values in differences.values()):
        raise RuntimeError("bootstrap unit is not exactly 1,600 episodes")
    bootstrap_rng = np.random.default_rng(BOOTSTRAP_SEED)
    sampling = bootstrap_rng.integers(
        0,
        V5_SAMPLE_SIZE,
        size=(BOOTSTRAP_REPLICATES, V5_SAMPLE_SIZE),
        dtype=np.int32,
    )
    bootstrap = {
    ...
    reconstructed_calls = sequential_calls(scores, thresholds)
    ...
    integrity = {
        "pre_v5_seal": True,
        "chronology": bool(
            sealed_inputs["all_1600_raw_and_execution_traces_complete_before_target_array_open"]
            and sealed_inputs["raw_manifest_precedes_execution"]
        ),
        "exact_episode_count": len(np.unique(episode_ids)) == V5_SAMPLE_SIZE,
        "exact_row_count": len(calls) == V5_SAMPLE_SIZE * 38,
        "finite_arrays": bool(
        ...
        and gate_flops_per == 7985,
        "seeded_comparator_weakly_more_total_compute": seeded_total_flops >= adaptive_total_flops,
        "gate_fit_selection_isolation": read_json(ROOT / "freeze/gate_freeze.json")["fit_selection_isolation_passed"],
        "confirmation_outcome_count_exact": execution_manifest["v5_outcome_episodes"]
        == V5_SAMPLE_SIZE,
    }
    stagewise_pass = all(item["positive_sign"] for item in stages)
    statistical_pass = (
        all(item["lower"] > 0 for item in individual.values())
    ...
    proposed = (
        ...
        if process_valid and statistical_pass
            ...
            "v5_confirmation_failed" if process_valid else "v5_execution_invalid"
        )
    )
    compute = {
        "rows": len(calls),
    ...
    atomic_npz(
        ROOT / "metrics/v5_confirmation_episode_metrics.npz",
        {
            "episode_id": np.arange(V5_SAMPLE_SIZE, dtype=np.int32),
