...
def prior_identifiers() -> tuple[set[int], set[str], dict[str, Any]]:
    ...
    snapshot = {
        "schema_version": 1,
        "scope": "all parseable prior run JSON and filenames outside V5 package v001",
    ...
    return [
        {
            "role": role,
            "slot": slot,
            "episode_id": f"v5v001-{label}-{slot:0{width}d}",
            "env_seed": base + slot,
...
def main() -> None:
    ...
    roles["replacement"] = records("replacement", 100, 1_951_900_000)
    for slot, item in enumerate(roles["replacement"]):
        item["episode_id"] = f"v5v001-replacement-{slot:03d}"
    analysis_seeds = {
    ...
    expected_numbers = 3 * sum(len(cohort) for cohort in roles.values()) + 3
    ...
    ledger = {
        ...
        **analysis_seeds,
        ...
        "freshness": "all package-smoke, confirmation, replacement, and analysis identifiers are disjoint from every recorded prior run identifier",
