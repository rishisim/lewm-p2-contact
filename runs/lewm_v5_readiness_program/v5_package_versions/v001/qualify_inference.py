...
from cycle_common import ROOT, V5_SAMPLE_SIZE, atomic_json, read_json
...
def main() -> None:
    ...
    rng = np.random.default_rng(26_071_500)
    ...
    checks = {
        ...
        "mixture_two_implementations_exact": bool(
            mixture["depth_lower"] == left
            and mixture["depth_upper"] == right
            and mixture["weight_upper"] == weight
            and np.allclose(mixture["loss"], independent, rtol=0.0, atol=1e-15)
        ),
