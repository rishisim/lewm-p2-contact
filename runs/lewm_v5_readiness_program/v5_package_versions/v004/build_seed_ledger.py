...
def prior_identifiers() -> tuple[set[int], set[str], dict[str, Any]]:
    ...
    snapshot = {
        "schema_version": 1,
        "scope": "all parseable prior run JSON and filenames outside V5 package v004",
    ...
    return [
        {
            "role": role,
            "slot": slot,
            "episode_id": f"v5v004-{label}-{slot:0{width}d}",
            "env_seed": base + slot,
...
def main() -> None:
    ...
    roles["replacement"] = records("replacement", 100, 2_730_900_000)
    for slot, item in enumerate(roles["replacement"]):
        item["episode_id"] = f"v5v004-replacement-{slot:03d}"
    analysis_seeds = {
        "bootstrap_seed": 2_831_999_991,
        "histogram_seed": 2_831_888_881,
        "seeded_mixture_seed": 2_831_777_771,
        "inference_qualification_seed": 2_831_666_661,
        "preseal_torch_seed": 2_831_555_551,
    }
    numbers = {
        int(item[key])
