...
class GenerationPreflightError(RuntimeError):
    """A package-level generation failure that consumes no seed tuple."""


def verify_pre_data() -> dict[str, Any]:
    from verify_pre_v5 import verify
...
def verify_role_permission(role: str) -> dict[str, Any]:
    if role not in ("package_smoke", "v5_confirmation"):
        raise RuntimeError("V5 package v004 allows only excluded package smoke or the frozen confirmation role")
    seal = verify_pre_data()
    if role == "v5_confirmation":
        ready_path = ROOT / "PACKAGE_READY.json"
        if not ready_path.exists():
            raise RuntimeError("confirmation execution is fail-closed until excluded package smoke finalizes PACKAGE_READY.json")
        ready = read_json(ready_path)
        if (
            ready.get("status") != "v5_package_ready"
            or not ready.get("excluded_package_smoke_passed")
            or ready.get("v5_outcome_episodes") != 0
        ):
            raise RuntimeError("PACKAGE_READY.json does not authorize the frozen confirmation role")
    return seal
...
def load_gate_tensors(torch: Any, device: Any) -> dict[str, Any]:
    with np.load(ROOT / "freeze/compiled_gate.npz", allow_pickle=False) as stored:
        gate = {
            ...
            # Scores and compiled affine heads are float32.  MPS does not support
            # float64 tensors, so comparisons use the score dtype on every device.
            "thresholds": torch.as_tensor(stored["thresholds"], dtype=torch.float32, device=device),
        }
    ...
    if gate["thresholds"].dtype != gate["a_raw"].dtype:
        raise RuntimeError("threshold and score dtype drift")
    return gate
...
def generation_preflight(role: str) -> tuple[dict[str, Any], Any, Any, Any, Any, Any]:
    """Validate the global DGP runtime and construct it before any tuple is touched."""
    world = None
    try:
        runtime = assert_runtime_contract("generation")
        seal = verify_role_permission(role)
        common, generator, seedfix = distribution_modules()
        world, policy = generator.make_world("plan_oracle")
        if world is None or policy is None or not callable(getattr(world, "close", None)):
            raise RuntimeError("PlanOracle DGP constructor returned an invalid world or policy")
        return runtime, seal, common, generator, seedfix, (world, policy)
    except BaseException as error:
        if world is not None:
            world.close()
        raise GenerationPreflightError(
            f"generation package preflight failed before any seed tuple: {error}"
        ) from error


def reconstruct_generation_world(generator: Any) -> tuple[Any, Any]:
    try:
        world, policy = generator.make_world("plan_oracle")
        if world is None or policy is None or not callable(getattr(world, "close", None)):
            raise RuntimeError("PlanOracle DGP reconstruction returned an invalid object")
        return world, policy
    except BaseException as error:
        raise GenerationPreflightError(
            f"global DGP reconstruction failed outside the replacement exception loop: {error}"
        ) from error


def record_generation_preflight_failure(role: str, error: BaseException) -> None:
    """Persist one package-level failure, never a per-seed replacement ledger."""
    output = ROOT / f"audit/{role}_generation_preflight_failure.json"
    if output.exists():
        return
    raw_directory = ROOT / f"data/{role}_raw"
    per_seed_failure = ROOT / f"audit/{role}_generation_failures.json"
    atomic_json(
        output,
        {
            "schema_version": 1,
            ...
            "failure_scope": "package_level_generation_preflight",
            "exception_type": type(error).__name__,
            "exception_message": str(error),
            "attempted_seed_tuple_count": 0,
            "primary_or_replacement_identifiers_consumed": 0,
            "role_raw_directory_exists": raw_directory.exists(),
            "per_seed_failure_ledger_exists": per_seed_failure.exists(),
            "outcome_loss_contact_reward_or_success_inspected": False,
        },
        exclusive=True,
    )
...
def generate(role: str) -> dict[str, Any]:
    # Interpreter, imports, exact versions, seal, and the global PlanOracle DGP
    # all pass before output directories, failure ledgers, or tuples are touched.
    runtime, seal, common, generator, seedfix, constructed = generation_preflight(role)
    world, policy = constructed
        ...
        for index, primary in enumerate(specs):
            ...
            for candidate, replacement_used in candidates:
                ...
                if seed_tuple in failed_seed_tuples or (replacement_used and seed_tuple in used_replacements):
                    continue
                if world is None:
                    world, policy = reconstruct_generation_world(generator)
                try:
                    ...
                    record = {
                        ...
                        "v5_outcome_episode": role == "v5_confirmation",
                    }
    ...
    manifest = {
        ...
        "v5_outcomes_inspected_during_generation": False,
        "v3_test_targets_opened": False,
        "released_hdf5_opened": False,
        "v5_outcome_episodes": expected if role == "v5_confirmation" else 0,
        "generation_runtime": runtime,
        "pre_v5_seal_sha256": seal["seal_sha256"],
        "global_generation_preflight_completed_before_output_directory": True,
    }
...
def main() -> None:
    ...
    generate_parser.add_argument("role", choices=("package_smoke", "v5_confirmation"))
    execute_parser = subparsers.add_parser("execute")
    execute_parser.add_argument("role", choices=("package_smoke", "v5_confirmation"))
    ...
    if arguments.command == "generate":
        try:
            ...
            record_generation_preflight_failure(arguments.role, error)
            raise
    else:
